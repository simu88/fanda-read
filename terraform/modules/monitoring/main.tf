...
# ===================================================================
# 2. Prometheus 스택 설치 (ALB 경로 설정 포함)
# ===================================================================
resource "helm_release" "prometheus" {
  ...
  version       = "57.0.1" # 널리 사용되는 안정적인 버전으로 조정
  recreate_pods = true
...
resource "helm_release" "opentelemetry_collector" {
  ...
  values = [
    yamlencode({
      ...
      config : {
        exporters : {
          # Loki가 제외되었으므로 해당 exporter 설정 제거
...
# ===================================================================
# 7. 단일 ALB를 위한 Kubernetes Ingress 생성 (어노테이션 오타 수정)
# 5. 모니터링용 Ingress (fanda-monitoring 네임스페이스, 지정 IP 허용 ALB)
# ===================================================================
# 5. 모니터링용 Ingress (fanda-monitoring 네임스페이스, 지정 IP 허용 ALB)
# 사용자께서 제공해주신 fanda_ingress의 구문을 정확히 참조하여 수정했습니다.
resource "kubernetes_ingress_v1" "monitoring_ingress" {
  provider = kubernetes # provider 명시적 지정

  metadata {
    name      = "monitoring-ingress"
    namespace = kubernetes_namespace.monitoring.metadata[0].name
    annotations = {
      # 이 Ingress가 생성할 두 번째 ALB에 대한 설정
      "alb.ingress.kubernetes.io/scheme" : "internet-facing"
      "alb.ingress.kubernetes.io/target-type" : "ip"

      # 이 ALB에만 적용될 IP 접근 제한 규칙
      "alb.ingress.kubernetes.io/inbound-cidrs" : "118.218.200.33/32, 58.78.119.14/32, 211.179.27.76/32"
    }
  }

  spec {
    # [핵심] 최신 방식으로 Ingress Class를 명시적으로 지정
    ingress_class_name = "alb"

    rule {
      http {
        # --- [핵심] fanda_ingress와 동일한, 올바른 HCL 중첩 블록 구문으로 수정 ---
        path {
          path      = "/grafana"
          path_type = "Prefix"
          backend {
